...
**Check ignores first:** run `git check-ignore -v .claude/evals/x`, because many repos ignore `.claude/`
...
edit the user's `.gitignore` for this. Note that a plugin does not carry `CLAUDE.md`, so cases that
