...
for (const e of await fs.readdir(dir, { withFileTypes: true })) {
  ...
  if (!f) continue;
  try { const j = JSON.parse(await fs.readFile(f, 'utf8')); if (!j.cases) continue; const m = id.match(/cc([\d.]+)/); runs.push({ id, at: j.generatedAt ?? id, ccVersion: m ? m[1] : null, runner: j.shim ? 'shim' : 'official', json: j, report: reportsDir && existsSync(path.join(reportsDir, id + '.html')) ? id + '.html' : null }); } catch {}
}
...
const target = out ?? path.join(path.resolve(dir), 'dashboard.html');
await fs.writeFile(target, html); console.log(target);
